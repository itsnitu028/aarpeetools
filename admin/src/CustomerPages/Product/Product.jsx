...
const Product = () => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:4000/customers/products');
      if (!response.ok) {
  ...
  // Get product price (for sorting) - moved before usage
  ...
  const filteredProducts = products.filter(product => {
    ...
    // Category filtering - handle multiple possible category formats
    let matchesCategory = selectedCategory === 'all';
    if (selectedCategory !== 'all' && product.category) {
